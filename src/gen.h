...
class IBGenerator : public cSimpleModule
{
 private:
  ...
  unsigned int flitSize_B; // number of bytes in each credit/FLIT，这个参数是干啥？？没看懂过滤大小？
  ...
  std::vector< IBAppMsg *> appMsgs; // requested messages by app port           // APP需求的消息，即是发送给APP的消息
