...
vecFile::parse(string fileName, int isInt) {
...
#endif
  char *tok;
  FILE *f = fopen(fileName.c_str(), "r");   //读文件
  if (f == NULL) {
  ...
  while (fgets(buf, 1024, f) != NULL) {
    ...
    tok = strtok_r(buf, " ,", &lasts); // 此时tok应该是0： 或者 1：....
    // get the index
...
vecFiles::parseNewFile(string fileName, int isInt) {
  ...
  if (vf->parse(fileName, isInt)) { //将fdbs数据写入VF中的 intData 或者FloatData vector数组中
    delete vf;
    return NULL;
  }
